# %% [markdown]
# # Pipeline v88 — Weighted Ensemble + Cross-Modal (MFCC+W2V) + Stacking | Target >= 0.75
#
# ─────────────────────────────────────────────────────────────────────────────
# v87 Breakthrough:
...
#
# STRATEGI v88 — CLOSE THE 0.0006 GAP:
...
RESULTS_DIR = os.path.join(PROJECT_ROOT, "results", "v88")
for d in [os.path.join(RESULTS_DIR, "metrics"), os.path.join(RESULTS_DIR, "plots")]:
    os.makedirs(d, exist_ok=True)

t_global = time.time()
print("=" * 80)
print("  Pipeline v88 — Weighted Ensemble + Cross-Modal + Stacking")
print("  Target: F1 >= 0.75 (gap only 0.0006 from v87!)")
print("=" * 80)
...
all_results = []
current_best = 0.7494   # v87 reference

print(f"\n{'='*80}")
print(f"  v88 MAIN LOOP: S1-S4 × 5 Models (OOF Threshold)")
print(f"  v87 Reference: 0.7494 | gap = 0.0006")
print(f"{'='*80}")
...
# Retrain best W2V models from v88 main loop
...
df_res.to_csv(os.path.join(RESULTS_DIR, "metrics", "v88_results.csv"), index=False)

sorted_res = sorted(all_results, key=lambda x: x['test_f1_oof'], reverse=True)

print(f"\n{'='*110}")
print(f"{'TABEL RINGKASAN v88 — S1-S4 × 5 Models (OOF Threshold)':^110}")
print(f"{'='*110}")
...
overall_best_v88 = max(current_best, best_stack_f1, best_weighted_f1)
for ax,title in[(ax1,'CV F1 (10-Fold)'),(ax2,'Test F1 (OOF Threshold)')]:
    ...
    ax.axhline(0.7494,color='orange',ls=':',lw=1.5,label='v87 Best 0.7494')
...
fig.suptitle(f'v88 — Weighted+CrossModal+Stacking\n'
             f'Best={overall_best_v88:.4f} | v87 ref=0.7494 | Target=0.75',
             fontsize=11,fontweight='bold')
plt.tight_layout()
fig.savefig(os.path.join(RESULTS_DIR,"plots","v88_comparison.png"),dpi=150,bbox_inches='tight')
plt.close()
...
print(f"\n{'='*80}")
print(f"{'FINAL REPORT v88':^80}")
print(f"{'='*80}")
print(f"  v87 Referensi    : 0.7494")
print(f"  v88 Best Single  : {best_single['test_f1_oof']:.4f} ({best_single['model']}_{best_single['scenario']})")
print(f"  v88 Weighted Ens : {best_weighted_f1:.4f}")
print(f"  v88 Stack        : {best_stack_f1:.4f}")
if cross_results:
    best_cr = max(cross_results, key=lambda k: cross_results[k]['f1_oof'])
    print(f"  v88 CrossModal   : {cross_results[best_cr]['f1_oof']:.4f} ({best_cr[:40]})")
print(f"  OVERALL BEST     : {overall_best_v88:.4f}")
print(f"\n  TARGET 0.75      : {'✓ TERCAPAI!' if overall_best_v88 >= 0.75 else f'NO (gap: {0.75-overall_best_v88:.4f})'}")
...
summary = {
    ...
    'overall_best': round(overall_best_v88, 4),
    'target_075': bool(overall_best_v88 >= 0.75),
    'v87_ref': 0.7494,
}
json.dump(summary, open(os.path.join(RESULTS_DIR,"metrics","v88_summary.json"),'w'), indent=2, cls=NumpyEncoder)
print(f"  Summary saved: {RESULTS_DIR}/metrics/v88_summary.json")
