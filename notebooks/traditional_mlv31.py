...
# **Pipeline v31** — Klasifikasi Kesehatan Mental Berbasis Audio
#
# ─────────────────────────────────────────────────────────────────────
#  v31 = LOOCV + SMOTE + Random Forest Feature Selection + Early Fusion + Ensemble
#
...
#  [4] Menggunakan RF-based Feature Selection (Non-linear) di dalam LOOCV
...
MODELS_DIR  = os.path.join(PROJECT_ROOT, "models", "ml_v31")
RESULTS_DIR = os.path.join(PROJECT_ROOT, "results", "v31")
...
def loocv_evaluate(df, feat_cols, model_fn):
    ...
    for i in range(n):
        ...
        rf_model = RandomForestClassifier(n_estimators=50, max_depth=5, random_state=RANDOM_SEED, class_weight='balanced', n_jobs=-1)
        sel = SelectFromModel(rf_model, threshold='1.25*median')
...
csv_path = os.path.join(RESULTS_DIR, "metrics", "v31_results.csv")
df_results.to_csv(csv_path, index=False)
print("\n" + "=" * 80)
print(f"RINGKASAN v31 — LOOCV (102 folds, SMOTE + RF Feature Selection + Early Fusion + Ensemble)")
print("=" * 80)
