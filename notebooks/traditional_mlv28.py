...
# **Pipeline v28** — Klasifikasi Kesehatan Mental Berbasis Audio
#
# ─────────────────────────────────────────────────────────────────────
#  v28 = LOOCV + SMOTE + SelectFromModel (Lasso) + Early Fusion + Ensemble
#
...
#  [4] Menggunakan L1-based Feature Selection (Lasso) di dalam LOOCV
...
MODELS_DIR  = os.path.join(PROJECT_ROOT, "models", "ml_v28")
RESULTS_DIR = os.path.join(PROJECT_ROOT, "results", "v28")
...
# %% [markdown]
# ## Stacking Meta-Learner (LOOCV over Out-Of-Fold Probabilities)

# %%
# Extract out-of-fold probabilities
meta_features = []
combo_names = list(all_results.keys())
for combo in combo_names:
    meta_features.append(all_ys[combo][1]) # y_prob

X_meta = np.column_stack(meta_features)
y_meta = all_ys[combo_names[0]][0]

n_meta = len(y_meta)
y_pred_meta, y_prob_meta = np.zeros(n_meta, dtype=int), np.zeros(n_meta, dtype=float)

for i in range(n_meta):
    X_tr_meta = np.delete(X_meta, i, axis=0)
    y_tr_meta = np.delete(y_meta, i, axis=0)
    X_te_meta = X_meta[i:i+1]
    
    meta_model = LogisticRegression(random_state=RANDOM_SEED, class_weight='balanced', C=1.0)
    meta_model.fit(X_tr_meta, y_tr_meta)
    
    prob = meta_model.predict_proba(X_te_meta)[0, 1]
    y_prob_meta[i] = prob

best_thr_meta, best_f1_meta = 0.5, f1_score(y_meta, (y_prob_meta >= 0.5).astype(int), average='macro', zero_division=0)
for thr in np.arange(0.30, 0.71, 0.01):
    preds = (y_prob_meta >= thr).astype(int)
    f1_t = f1_score(y_meta, preds, average='macro', zero_division=0)
    if f1_t > best_f1_meta:
        best_f1_meta, best_thr_meta = f1_t, thr

all_results['Ensemble_StackingLR'] = {
    'f1_tuned': best_f1_meta, 'best_threshold': best_thr_meta,
    'roc_auc': roc_auc_score(y_meta, y_prob_meta)
}
...
csv_path = os.path.join(RESULTS_DIR, "metrics", "v28_results.csv")
df_results.to_csv(csv_path, index=False)
print("\n" + "=" * 80)
print(f"RINGKASAN v28 — LOOCV (102 folds, SMOTE + Lasso FS + Early Fusion + Stacking Meta-Learner)")
print("=" * 80)
