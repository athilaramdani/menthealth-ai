...
# **Pipeline v39** — Klasifikasi Kesehatan Mental Berbasis Multimodal Stacking
#
# ─────────────────────────────────────────────────────────────────────
#  v39 = LOOCV + SMOTE + Block-wise Lasso FS + Audio/Text + Stacking
#
...
#  [4] Model ke-5 adalah **StackingClassifier** yang menjadikan 4 model dasar sebagai
#      penggali fitur awal, lalu memprediksi final menggunakan LR Meta-Learner.
#  [5] Menggunakan Block-wise Lasso Feature Selection
# ─────────────────────────────────────────────────────────────────────
...
MODELS_DIR  = os.path.join(PROJECT_ROOT, "models", "ml_v39")
RESULTS_DIR = os.path.join(PROJECT_ROOT, "results", "v39")
...
def get_base_estimators():
    return [
        ('lr', LogisticRegression(max_iter=10000, random_state=RANDOM_SEED, class_weight='balanced', C=0.01, solver='liblinear')),
        ('svm', SVC(kernel='rbf', probability=True, C=0.5, gamma='scale', random_state=RANDOM_SEED, class_weight='balanced')),
        ('xgb', xgb.XGBClassifier(random_state=RANDOM_SEED, eval_metric='logloss', objective='binary:logistic', n_jobs=-1, scale_pos_weight=2.5, n_estimators=100, max_depth=3, learning_rate=0.05, subsample=0.8, colsample_bytree=0.8)),
        ('rf', RandomForestClassifier(random_state=RANDOM_SEED, class_weight='balanced', n_jobs=-1, n_estimators=300, max_depth=5, max_features='sqrt'))
    ]

def get_models():
    models = {name: est for name, est in get_base_estimators()}
    # Model ke-5: Stacking Meta-Learner (Menggunakan 4 model di atas sebagai pemikir awal)
    models['Stacking'] = StackingClassifier(
        estimators=get_base_estimators(),
        final_estimator=LogisticRegression(random_state=RANDOM_SEED, class_weight='balanced', C=0.1),
        cv=5,
        n_jobs=-1
    )
    return models
...
csv_path = os.path.join(RESULTS_DIR, "metrics", "v39_results.csv")
df_results.to_csv(csv_path, index=False)
print("\n" + "=" * 80)
print(f"RINGKASAN v39 — LOOCV (Full Multimodal + Text + Audio + Stacking Classifier + Klasik)")
print("=" * 80)
