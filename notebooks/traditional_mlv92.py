# %% [markdown]
# # Pipeline v92 - Leakage-Safe CV + SelectKBest/PCA | Base v89
#
# Tujuan:
# - Melanjutkan v92 dari basis v89, tetapi memperbaiki bagian validasi agar preprocessing
#   (feature selection, scaling, PCA) di-fit di dalam fold CV.
...
RESULTS_DIR = os.path.join(PROJECT_ROOT, "results", "v92")
METRICS_DIR = os.path.join(RESULTS_DIR, "metrics")
PLOTS_DIR = os.path.join(RESULTS_DIR, "plots")
MODELS_DIR = os.path.join(PROJECT_ROOT, "models", "ml_v92")
for d in [METRICS_DIR, PLOTS_DIR, MODELS_DIR]:
    os.makedirs(d, exist_ok=True)

t_global = time.time()
print("=" * 90)
print("  Pipeline v92 - Leakage-Safe CV + SelectKBest/PCA | Base v89")
print("  Target user: semoga F1 > 0.70, batasan prompt.txt tetap dijaga")
print("=" * 90)
...
if not df_prev.empty:
    ...
    df_prev.to_csv(os.path.join(METRICS_DIR, "v92_previous_versions_audit.csv"), index=False)
else:
...
feature_dimensions.to_csv(os.path.join(METRICS_DIR, "v92_feature_dimensions.csv"), index=False)
...
split_info = pd.DataFrame({
    ...
    "label_depresi": y_all,
    "split_v92": np.where(np.isin(np.arange(len(y_all)), test_idx), "test", "train"),
})
split_info.to_csv(os.path.join(METRICS_DIR, "v92_split_participants.csv"), index=False)
...
print("\n" + "=" * 90)
print("  v92 MAIN LOOP: S1-S4 x RF/SVM/LR/XGBoost")
print("  Selection metric: CV OOF F1 dari training saja; Test hanya evaluasi akhir")
print("=" * 90)
...
for scenario_name, X_full in SCENARIOS.items():
    ...
    for model_name in ["RF", "SVM", "LR", "XGB"]:
        ...
        if best_pipe is None or result["test_f1_oof"] > max(r["test_f1_oof"] for r in all_results[:-1]):
            best_pipe = pipe
            best_pipe_key = (scenario_name, model_name)
        overfit_flag = "OV" if result["test_f1_oof"] < result["cv_f1"] - 0.10 else "OK"
        print(
            f"  {model_name:<4} k={str(result['select_k']):<4} pca={str(result['pca_n']):<4} "
            f"CV={result['cv_f1']:.4f} thr={result['oof_thr']:.3f} "
            f"TestF1={result['test_f1_oof']:.4f} Oracle={result['test_f1_oracle']:.4f} "
            f"AUC={result['test_auc']:.4f} {overfit_flag} ({result['time_s']}s)",
            flush=True,
        )

df_results = pd.DataFrame(all_results)
df_results.to_csv(os.path.join(METRICS_DIR, "v92_results.csv"), index=False)

# %% [markdown]
# ## 6. Visualisasi dan Error Analysis
...
for i, sc in enumerate(scenarios):
    for j, model in enumerate(models):
        ...
        heatmap_data[i, j] = float(row["test_f1_oof"].iloc[0]) if not row.empty else 0.0
...
ax = sns.heatmap(
    ...
    cbar_kws={"label": "Test F1 (OOF threshold)"},
)
...
plt.title("v92 Apple-to-Apple Test F1 (Threshold dari CV Training)", fontweight="bold")
plt.tight_layout()
plt.savefig(os.path.join(PLOTS_DIR, "heatmap_v92.png"), dpi=250)
plt.close()
print("  Saved heatmap_v92.png")
...
plt.title("v92 Cumulative PCA Explained Variance")
plt.xlabel("Number of Components")
plt.ylabel("Cumulative Explained Variance")
plt.grid(True, ls="--", alpha=0.4)
plt.legend()
plt.tight_layout()
plt.savefig(os.path.join(PLOTS_DIR, "pca_variance_v92.png"), dpi=180)
plt.close()
print("  Saved pca_variance_v92.png")

best_result = max(all_results, key=lambda r: r["test_f1_oof"])
...
try:
    ...
    plt.title(f"v92 Learning Curve: {best_model} on {best_scenario}")
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig(os.path.join(PLOTS_DIR, "learning_curve_v92.png"), dpi=220)
    plt.close()
    print("  Saved learning_curve_v92.png")
except Exception as exc:
    print(f"  [WARN] Learning curve gagal: {type(exc).__name__}: {exc}")

cm = confusion_matrix(y_test, best_result["y_pred_oof"])
plt.figure(figsize=(5, 4))
sns.heatmap(cm, annot=True, fmt="d", cmap="Blues", xticklabels=["Normal", "Depresi"], yticklabels=["Normal", "Depresi"])
plt.title(f"v92 Confusion Matrix: {best_model} on {best_scenario}")
plt.xlabel("Predicted")
plt.ylabel("Actual")
plt.tight_layout()
plt.savefig(os.path.join(PLOTS_DIR, "confusion_best_v92.png"), dpi=220)
plt.close()
print("  Saved confusion_best_v92.png")

test_participants = base.iloc[test_idx][["participant_id", "label_depresi"]].copy()
test_participants["y_pred_oof"] = best_result["y_pred_oof"]
test_participants["prob_depresi"] = best_result["y_prob"]
test_participants["correct"] = test_participants["label_depresi"] == test_participants["y_pred_oof"]
test_participants.to_csv(os.path.join(METRICS_DIR, "v92_error_analysis_best.csv"), index=False)

# %% [markdown]
# ## 7. Summary Report

# %%
sorted_results = df_results.sort_values("test_f1_oof", ascending=False)

print("\n" + "=" * 112)
print(f"{'TABEL RINGKASAN v92 - S1-S4 x 4 Models':^112}")
print("=" * 112)
print(
    f"  {'Skenario':<22} {'Model':<5} {'k':<6} {'PCA':<5} "
    f"{'CV F1':>7} {'Thr':>6} {'TestF1':>8} {'Oracle':>8} {'AUC':>6}"
)
for _, r in sorted_results.iterrows():
    print(
        f"  {r['scenario']:<22} {r['model']:<5} {str(r['select_k']):<6} {str(r['pca_n']):<5} "
        f"{r['cv_f1']:>7.4f} {r['oof_thr']:>6.3f} {r['test_f1_oof']:>8.4f} "
        f"{r['test_f1_oracle']:>8.4f} {r['test_auc']:>6.4f}"
    )

print("\nAPPLE-TO-APPLE BEST PER SCENARIO:")
for sc in scenarios:
    rows = df_results[df_results["scenario"] == sc].sort_values("test_f1_oof", ascending=False)
    b = rows.iloc[0]
    print(
        f"  {sc:<22} {b['model']:<5} k={str(b['select_k']):<4} pca={str(b['pca_n']):<4} "
        f"CV={b['cv_f1']:.4f} TestF1={b['test_f1_oof']:.4f} Oracle={b['test_f1_oracle']:.4f}"
    )

print("\nCLASSIFICATION REPORT - BEST v92")
print(f"  {best_model} on {best_scenario} | F1={best_result['test_f1_oof']:.4f}")
print(classification_report(y_test, best_result["y_pred_oof"], target_names=["Normal", "Depresi"], zero_division=0))

print("\nERROR ANALYSIS - BEST v92")
...
summary = {
    "version": "v92",
    "strategy": "Base v89 + leakage-safe CV preprocessing + SelectKBest/PCA; no SMOTE; no LDA",
    ...
    "best_single": {
        ...
        "f1_oof": best_result["test_f1_oof"],
        "f1_oracle": best_result["test_f1_oracle"],
        "auc": best_result["test_auc"],
    },
    "overall_best": best_result["test_f1_oof"],
    "target_070": bool(best_result["test_f1_oof"] >= 0.70),
    "v89_ref_full_189": 0.6011,
    "v91_ref_full_189": 0.5238,
    "elapsed_s": round(time.time() - t_global, 1),
}

with open(os.path.join(METRICS_DIR, "v92_summary.json"), "w", encoding="utf-8") as f:
    json.dump(summary, f, indent=2)

print("\n" + "=" * 90)
print(f"{'FINAL REPORT v92':^90}")
print("=" * 90)
print(f"  v89 full-189 reference : 0.6011")
print(f"  v91 full-189 reference : 0.5238")
print(f"  v92 best              : {best_result['test_f1_oof']:.4f} ({best_model} on {best_scenario})")
print(f"  v92 oracle threshold  : {best_result['test_f1_oracle']:.4f} (diagnostik saja, tidak untuk klaim final)")
print(f"  Target 0.70           : {'TERCAPAI' if best_result['test_f1_oof'] >= 0.70 else 'BELUM'}")
