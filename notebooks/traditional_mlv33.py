...
# **Pipeline v33** — Klasifikasi Kesehatan Mental Berbasis Audio
#
# ─────────────────────────────────────────────────────────────────────
#  v33 = LOOCV + SMOTE + ElasticNet Feature Selection + Early Fusion + Ensemble
#
...
#  [4] Menggunakan ElasticNet Feature Selection untuk mempertahankan korelasi grup fitur audio
...
MODELS_DIR  = os.path.join(PROJECT_ROOT, "models", "ml_v33")
RESULTS_DIR = os.path.join(PROJECT_ROOT, "results", "v33")
...
def loocv_evaluate(df, feat_cols, model_fn):
    ...
    for i in range(n):
        ...
        # ElasticNet is superior for audio because it selects correlated groups of features
        # rather than randomly dropping all but one feature in a correlated group like pure Lasso.
        el_model = LogisticRegression(penalty='elasticnet', l1_ratio=0.5, solver='saga', C=0.1, max_iter=2000, random_state=RANDOM_SEED)
        sel = SelectFromModel(el_model)
...
csv_path = os.path.join(RESULTS_DIR, "metrics", "v33_results.csv")
df_results.to_csv(csv_path, index=False)
print("\n" + "=" * 80)
print(f"RINGKASAN v33 — LOOCV (102 folds, SMOTE + ElasticNet FS + Early Fusion + Ensemble)")
print("=" * 80)
