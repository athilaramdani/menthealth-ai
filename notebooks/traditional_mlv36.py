...
# **Pipeline v36** — Klasifikasi Kesehatan Mental Berbasis Audio
#
# ─────────────────────────────────────────────────────────────────────
#  v36 = LOOCV + SVMSMOTE + Block-wise Lasso FS + Early Fusion + Ensemble
#
...
#  [5] Menggunakan SVMSMOTE untuk mensintesis data persis di decision boundary
#  [6] Menambahkan Soft Voting Ensemble di akhir
# ─────────────────────────────────────────────────────────────────────
...
MODELS_DIR  = os.path.join(PROJECT_ROOT, "models", "ml_v36")
RESULTS_DIR = os.path.join(PROJECT_ROOT, "results", "v36")
...
def loocv_evaluate(df, feat_cols, model_fn):
    ...
    for i in range(n):
        ...
        # SVMSMOTE with 2 neighbors
        sm = SVMSMOTE(random_state=RANDOM_SEED, k_neighbors=2)
...
csv_path = os.path.join(RESULTS_DIR, "metrics", "v36_results.csv")
df_results.to_csv(csv_path, index=False)
print("\n" + "=" * 80)
print(f"RINGKASAN v36 — LOOCV (102 folds, SVMSMOTE + Block-wise Lasso FS + Early Fusion + Ensemble)")
print("=" * 80)
