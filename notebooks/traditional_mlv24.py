...
# **Pipeline v24** — Klasifikasi Kesehatan Mental Berbasis Audio
#
# ─────────────────────────────────────────────────────────────────────
#  v24 = LOOCV + SMOTE + Lasso FS (C=0.5) + RobustScaler + IQR Clip
#
...
#  [4] Menggunakan L1-based Feature Selection (Lasso C=0.5)
...
MODELS_DIR  = os.path.join(PROJECT_ROOT, "models", "ml_v24")
RESULTS_DIR = os.path.join(PROJECT_ROOT, "results", "v24")
...
def loocv_evaluate(df, feat_cols, model_fn):
    ...
    for i in range(n):
        ...
        IQR_val = Q3 - Q1
        lo, hi = Q1 - 10*IQR_val, Q3 + 10*IQR_val
        ...
        # SMOTE with 3 neighbors
        sm = SMOTE(random_state=RANDOM_SEED, k_neighbors=3)
        X_tr_res, y_tr_res = sm.fit_resample(X_tr, y_tr)
        
        # Lasso Feature Selection (C=0.5 for less penalty)
        l1_model = LogisticRegression(penalty='l1', solver='liblinear', C=0.5, random_state=RANDOM_SEED)
...
csv_path = os.path.join(RESULTS_DIR, "metrics", "v24_results.csv")
df_results.to_csv(csv_path, index=False)
print("\n" + "=" * 80)
print(f"RINGKASAN v24 — LOOCV (102 folds, SMOTE + Lasso FS C=0.5 + RobustScaler + IQR + Early Fusion + Ensemble)")
print("=" * 80)
