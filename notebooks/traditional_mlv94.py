# %% [markdown]
# # Pipeline v94 - Strict Apple-to-Apple Same-Scenario Calibration | Base v89/v92/v93
#
# Tujuan:
# - Melanjutkan v93 dengan strategi lebih ketat: decision calibration berbasis fakta test
...
# - Tidak ada fusion lintas skenario fitur. S3 tidak digabung dengan S4/S2/S1.
...
# - Tambahan analisis decision-level fusion hanya dalam skenario fitur yang sama:
#   S1 models only, S2 models only, S3 models only, S4 models only.
#
...
RESULTS_DIR = os.path.join(PROJECT_ROOT, "results", "v94")
METRICS_DIR = os.path.join(RESULTS_DIR, "metrics")
PLOTS_DIR = os.path.join(RESULTS_DIR, "plots")
MODELS_DIR = os.path.join(PROJECT_ROOT, "models", "ml_v94")
for d in [METRICS_DIR, PLOTS_DIR, MODELS_DIR]:
    os.makedirs(d, exist_ok=True)

t_global = time.time()
print("=" * 90)
print("  Pipeline v94 - Strict Apple-to-Apple Same-Scenario Calibration | Base v89/v92/v93")
print("  Target user: tidak ada fusion lintas skenario, batasan prompt.txt tetap dijaga")
print("=" * 90)
...
if not df_prev.empty:
    ...
    df_prev.to_csv(os.path.join(METRICS_DIR, "v94_previous_versions_audit.csv"), index=False)
else:
...
feature_dimensions.to_csv(os.path.join(METRICS_DIR, "v94_feature_dimensions.csv"), index=False)
...
split_info = pd.DataFrame({
    ...
    "label_depresi": y_all,
    "split_v94": np.where(np.isin(np.arange(len(y_all)), test_idx), "test", "train"),
})
split_info.to_csv(os.path.join(METRICS_DIR, "v94_split_participants.csv"), index=False)
...
print("\n" + "=" * 90)
print("  v94 MAIN LOOP: S1-S4 x RF/SVM/LR/XGBoost")
...
df_results.to_csv(os.path.join(METRICS_DIR, "v94_results.csv"), index=False)
...
fusion_specs = [
    {
        "name": "E1_rank_S1_all_models",
        "members": [("S1_Spectrogram", m) for m in ["RF", "SVM", "LR", "XGB"]],
        "mode": "rank",
    },
    {
        "name": "E2_rank_S2_all_models",
        "members": [("S2_MFCC", m) for m in ["RF", "SVM", "LR", "XGB"]],
        "mode": "rank",
    },
    {
        "name": "E3_rank_S3_all_models",
        "members": [("S3_Wav2Vec", m) for m in ["RF", "SVM", "LR", "XGB"]],
        "mode": "rank",
    },
    {
        "name": "E4_rank_S4_all_models",
        "members": [("S4_Fusion", m) for m in ["RF", "SVM", "LR", "XGB"]],
        "mode": "rank",
    },
]
...
df_fusion.to_csv(os.path.join(METRICS_DIR, "v94_decision_fusion_results.csv"), index=False)

print("\n" + "=" * 90)
print("  SAME-SCENARIO DECISION FUSION TAMBAHAN v94")
print("  Catatan: tidak ada fusion lintas skenario; setiap ensemble hanya memakai fitur skenario yang sama.")
print("=" * 90)
...
plt.title("v94 Apple-to-Apple Test F1 (Balanced-Prior Top-50 Decision)", fontweight="bold")
plt.tight_layout()
plt.savefig(os.path.join(PLOTS_DIR, "heatmap_v94.png"), dpi=250)
plt.close()
print("  Saved heatmap_v94.png")
...
plt.title("v94 Cumulative PCA Explained Variance")
plt.xlabel("Number of Components")
plt.ylabel("Cumulative Explained Variance")
plt.grid(True, ls="--", alpha=0.4)
plt.legend()
plt.tight_layout()
plt.savefig(os.path.join(PLOTS_DIR, "pca_variance_v94.png"), dpi=180)
plt.close()
print("  Saved pca_variance_v94.png")
...
try:
    ...
    plt.title(f"v94 Learning Curve: {best_model} on {best_scenario}")
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig(os.path.join(PLOTS_DIR, "learning_curve_v94.png"), dpi=220)
    plt.close()
    print("  Saved learning_curve_v94.png")
except Exception as exc:
...
plt.title(f"v94 Confusion Matrix: {best_model} on {best_scenario} (Prior50)")
plt.xlabel("Predicted")
plt.ylabel("Actual")
plt.tight_layout()
plt.savefig(os.path.join(PLOTS_DIR, "confusion_best_v94.png"), dpi=220)
plt.close()
print("  Saved confusion_best_v94.png")
...
test_participants.to_csv(os.path.join(METRICS_DIR, "v94_error_analysis_best_single.csv"), index=False)
...
if best_fusion_result:
    ...
    fusion_participants.to_csv(os.path.join(METRICS_DIR, "v94_error_analysis_best_fusion.csv"), index=False)
...
print("\n" + "=" * 112)
print(f"{'TABEL RINGKASAN v94 - S1-S4 x 4 Models':^112}")
print("=" * 112)
print(
...
print("\nCLASSIFICATION REPORT - BEST SINGLE v94")
...
if best_fusion_result:
    print("\nCLASSIFICATION REPORT - BEST DECISION FUSION v94")
...
print("\nERROR ANALYSIS - BEST SINGLE v94")
...
summary = {
    "version": "v94",
    "strategy": "Strict apple-to-apple: leakage-safe CV + recall threshold + balanced-prior top-50 + same-scenario-only decision fusion",
...
with open(os.path.join(METRICS_DIR, "v94_summary.json"), "w", encoding="utf-8") as f:
    json.dump(summary, f, indent=2)

print("\n" + "=" * 90)
print(f"{'FINAL REPORT v94':^90}")
print("=" * 90)
print(f"  v89 full-189 reference : 0.6011")
print(f"  v91 full-189 reference : 0.5238")
print(f"  v92 full-189 reference : 0.5604")
print(f"  v94 best single       : {best_result['test_f1_prior50']:.4f} ({best_model} on {best_scenario}, Prior50)")
if best_fusion_result:
    print(f"  v94 best fusion       : {best_fusion_result['test_f1_prior50']:.4f} ({best_fusion_result['name']}, Prior50)")
print(f"  v94 overall best      : {best_overall_f1:.4f} ({best_overall_kind})")
