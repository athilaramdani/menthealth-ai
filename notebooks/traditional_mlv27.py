...
# **Pipeline v27** — Klasifikasi Kesehatan Mental Berbasis Audio
#
# ─────────────────────────────────────────────────────────────────────
#  v27 = LOOCV + SMOTE + SelectFromModel (Lasso) + Early Fusion + Ensemble
#
...
MODELS_DIR  = os.path.join(PROJECT_ROOT, "models", "ml_v27")
RESULTS_DIR = os.path.join(PROJECT_ROOT, "results", "v27")
...
def loocv_evaluate(df, feat_cols, model_fn):
    ...
    for i in range(n):
        ...
        # NO SMOTE - Relying on class_weight=balanced
...
csv_path = os.path.join(RESULTS_DIR, "metrics", "v27_results.csv")
df_results.to_csv(csv_path, index=False)
print("\n" + "=" * 80)
print(f"RINGKASAN v27 — LOOCV (102 folds, NO SMOTE + Lasso FS + Early Fusion + Ensemble)")
print("=" * 80)
