...
# **Pipeline v52** — eGeMAPS + MFCC + BERT + LINGUISTIC: MAX FUSION
#
# ─────────────────────────────────────────────────────────────────────
#  v52 = v51 Best Lessons + eGeMAPS Features + Expanded Ensemble
#
#  Error Analysis v51:
#  - Best Top-5 Ensemble: F1=0.7756 >= 0.75 ✅
#  - Depressed: precision=0.67, recall=0.71, F1=0.69
#  - Non-Depressed: precision=0.88, recall=0.85, F1=0.86
#  - Gap to 0.80: only 0.024 — need to push depressed F1 ~0.74+
#  - Best CV: MFCC_Ling|SVM_rbf_C2 (CV F1=0.726)
#  - Top-5 soft vote was KEY: individual ~0.68, ensemble ~0.78
#
#  Strategi v52 (Target >= 0.80):
#  [1] eGeMAPS v15 (88 features, acoustic+prosodic, research-standard)
#      - F0, loudness, MFCC1-4, jitter, shimmer, HNR, formants (F1-F3)
#      - alphaRatio, hammarbergIndex, spectralFlux, voiced/unvoiced stats
#  [2] MFCC dari daic_features_final.csv (56D, coverage 189/189)
#  [3] BERT Embeddings dari v13 (384D)
#  [4] Linguistic Features (25D)
#  [5] Feature-level SelectKBest untuk tiap blok audio
#  [6] Top-10 diverse ensemble dengan fitur berbeda
#  [7] Original DAIC-WOZ split (proven best protocol)
#  [8] Calibrated classifiers untuk better probability calibration
# ─────────────────────────────────────────────────────────────────────
...
V14_DIR   = os.path.join(PROJECT_ROOT, "data", "features", "v14")
V15_DIR   = os.path.join(PROJECT_ROOT, "data", "features", "v15")
V49_CACHE = os.path.join(PROJECT_ROOT, "results", "v49", "metrics")
RESULTS_DIR = os.path.join(PROJECT_ROOT, "results", "v52")
...
# %% [markdown]
# ## Feature 2: MFCC + Acoustic (daic_features_final.csv, 56D, coverage 189/189)
...
# %% [markdown]
# ## Feature 3: eGeMAPS v15 (88 standard acoustic features)

# %%
print("\nLoading eGeMAPS v15 features...")
df_egemap = pd.read_csv(os.path.join(V15_DIR, "daic_v15_egemaps_full.csv"))
print(f"  eGeMAPS shape: {df_egemap.shape}")

# Identify columns
egemap_meta = ['participant_id', 'label']
egemap_feat_cols = [c for c in df_egemap.columns if c not in egemap_meta]
print(f"  eGeMAPS feature columns: {len(egemap_feat_cols)}")
print(f"  Coverage: {len(df_egemap)} participants")

# Merge with labels
df_labels = df_labels.merge(
    df_egemap[['participant_id'] + egemap_feat_cols],
    left_on='id', right_on='participant_id', how='left', suffixes=('', '_egemap')
)
X_egemap = df_labels[egemap_feat_cols].fillna(0).values.astype(np.float64)
X_egemap = np.nan_to_num(X_egemap, nan=0.0, posinf=0.0, neginf=0.0)
np.clip(X_egemap, -1e6, 1e6, out=X_egemap)
egemap_coverage = int((X_egemap.sum(axis=1) != 0).sum())
print(f"  eGeMAPS: {X_egemap.shape} | coverage: {egemap_coverage}/189")

# %% [markdown]
# ## Feature 4: Linguistic Features (25D, from transcripts)
...
# All feature combos — including eGeMAPS
feature_sets = {
    'BERT_Ling':             np.hstack([X_bert, X_ling]),
    'MFCC_Ling':             np.hstack([X_mfcc, X_ling, X_gender]),
    'eGeMAPS_Ling':          np.hstack([X_egemap, X_ling]),
    'BERT_eGeMAPS_Ling':     np.hstack([X_bert, X_egemap, X_ling]),
    'eGeMAPS_MFCC_Ling':     np.hstack([X_egemap, X_mfcc, X_ling]),
    'BERT_MFCC_Ling':        np.hstack([X_bert, X_mfcc, X_ling]),
    'All_Features':          np.hstack([X_bert, X_egemap, X_mfcc, X_ling, X_pros, X_gender]),
}
for k, v in feature_sets.items():
    print(f"  {k}: {v.shape}")

# %% [markdown]
# ## Model Grid (SVM + LR for CV — lean)
...
# %% [markdown]
# ## Diverse Ensemble Strategies (Top-3, 5, 7, 10)

# %%
print("\n--- Soft Voting Ensembles ---")
best_ens_f1 = 0.0; best_ens_probs = None; best_ens_thr = 0.5; best_ens_k = 5
...
for k_top in [3, 5, 7, 10, 15]:
    ...
    print(f"  Top-{k_top:<2} Ensemble: F1={best_f1_e:.4f} Acc={acc_e:.4f} AUC={auc_e:.4f} (thr={best_thr_e:.2f})")
    if best_f1_e > best_ens_f1:
        best_ens_f1 = best_f1_e; best_ens_probs = ens
        best_ens_thr = best_thr_e; best_ens_k = k_top
...
if best_ens_probs is not None and test_probs:
    top5 = top_by_f1[:5]
    w5   = np.array([test_probs[k][2] for k in top5]); w5 /= w5.sum()
...
# %%
print("\n" + "="*70)
print("FINAL SUMMARY v52 — eGeMAPS + MFCC + BERT + LINGUISTIC")
print("="*70)
...
df_cv.to_csv(os.path.join(RESULTS_DIR, "metrics", "v52_cv_results.csv"))
