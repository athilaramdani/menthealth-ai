...
# **Pipeline v25** — Klasifikasi Kesehatan Mental Berbasis Audio
#
# ─────────────────────────────────────────────────────────────────────
#  v25 = LOOCV + SMOTE + SelectFromModel (Lasso) + Early Fusion + Ensemble
#
...
MODELS_DIR  = os.path.join(PROJECT_ROOT, "models", "ml_v25")
RESULTS_DIR = os.path.join(PROJECT_ROOT, "results", "v25")
...
def get_models():
    return {
        'Logistic Regression': LogisticRegression(max_iter=10000, random_state=RANDOM_SEED, class_weight='balanced', C=1.0, solver='liblinear'),
        'SVM': SVC(kernel='rbf', probability=True, C=1.0, gamma='scale', random_state=RANDOM_SEED, class_weight='balanced'),
        'XGBoost': xgb.XGBClassifier(random_state=RANDOM_SEED, eval_metric='logloss', objective='binary:logistic', n_jobs=-1, scale_pos_weight=2.5, n_estimators=500, max_depth=10, learning_rate=0.1, subsample=0.8, colsample_bytree=0.8),
        'Random Forest': RandomForestClassifier(random_state=RANDOM_SEED, class_weight='balanced', n_jobs=-1, n_estimators=500, max_depth=10, max_features='sqrt')
    }
...
csv_path = os.path.join(RESULTS_DIR, "metrics", "v25_results.csv")
df_results.to_csv(csv_path, index=False)
print("\n" + "=" * 80)
print(f"RINGKASAN v25 — LOOCV (102 folds, SMOTE + Lasso FS + Early Fusion + Ensemble)")
print("=" * 80)
